...
const Author = ({ pageContext, data }) => {
  ...
  //const { limited } = this.data.limit
  const { totalCount } = data.allMarkdownRemark
  //const { github } = data.allMarkdownRemark.edges.node.frontmatter.github
  ...
  return (
    ...
    <div className={style.wrapper}>
      ...
      {data.allMarkdownRemark.edges.slice(0, 1).map(({ node }) => (
            ...
            <div className={style.profileSection}>
              <div className={style.details}>
                ...
                <div className={style.follow}>
                  <a className={style.btn} href={node.frontmatter.github}>Follow</a>
                </div>
              </div>
            </div>
            <div className={style.profileSection}>
              <div className={style.linksWrapper}>
                ...
                <div className={style.links}>
                  <a href={node.frontmatter.github} className={style.link}>
...
export const authorQuery = graphql`
  query($authors: String) {
    allMarkdownRemark( filter: { frontmatter: { author: { in: [$authors] } } } sort: { fields: [frontmatter___title], order: ASC}) {
      totalCount
      edges {
        node {
          excerpt
